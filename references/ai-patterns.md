...
**检测：** 一段话中出现 **2 个及以上**"——"（即每段超过 1 个即超标）。

**处理：** 每段最多保留 1 个破折号，其余替换为逗号、句号或括号。此阈值与 `replacement-tables.md`、`SKILL.md` 第三轮审计一致。破折号是 DeepSeek 等模型的强口癖，宁严勿宽。
...
AI 文本经常用成对否定或让步完成段落收束。单个句式不是判断依据，连续多个段落采用相同结构时才需要处理。
...
**处理：** 删除用于制造转折感的外壳，保留具体事实。优先说明原因、观测结果或验证边界，不要把同一段重新改成另一组成对连接词。

- "样本少不代表研究价值低，而是投入资源不足。"
- → "样本数量少以后，研究经费和项目数量也随之减少。"
