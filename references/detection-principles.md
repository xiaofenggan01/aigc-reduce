...
### ⚠️ 护栏：不要把"反直觉发现"误读成"写差一点更安全"

上面的数据是**统计观察，不是操作指南**。一个致命的误读是：既然"口语化不规范处"能把假阳性率压到 1.8%，那就该主动把论文写得口语、写差一点。**这是错的**，原因有三：

1. **过得了机审，过不了人审。** 导师、评审、答辩委员会一眼就能看出口语化、网络用语、破折号泛滥的文本不是合格论文。降重的目标是让论文**既通过 AI 检测、又是一篇合格论文**，不是只骗过机器。
2. **违背学术规范。** 学术写作有明确的语体要求（书面、正式、术语稳定、方法描述中的被动语态合规）。为降 AI 率牺牲这些，本末倒置。
3. **那 1.8% 是副作用，不可复制。** 它来自真实作者无意识的个人化表达；刻意模仿只会产出"假口语"——既不像真人，也不像论文，反而可能被新一代语义检测识别。

**正确方向**：降 AI 率靠**打破模板化的句式结构**——语序重组、句式变换、主被动互换、长句拆分、论证组织调整、模板套话去除（见第五节"深度修改"和"语义重构"）。这些操作在**不破坏学术语体**的前提下即可把修改率推到 40% 以上。**语体是硬底线，结构改写是手段。** 正向标准与"降重不足 / 过度口语 / 恰当"三级对照见 `positive-style-academic.md`。
...
| 中度修改 (20-40%) | 35-50% | 句式重构、补全已有细节、书面化的推断引导词（非口语） |
| 深度修改 (>40%) | 60-80% | 语义重构、书面化的作者判断、打破模板结构（全程保持学术语体） |
